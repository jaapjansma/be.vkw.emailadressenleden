...
class CRM_Emailadressenleden_Form_Report_EmailadressenLedenEvent extends CRM_Report_Form {
  ...
  function __construct() {
    $this->_columns = array(
        ...
        'filters' => array(
          'sid' => array(
           ...
           // 'type' => CRM_Utils_Type::T_INT,
			...
			'default' => CRM_Utils_Array::implodePadded(array_keys(CRM_Member_PseudoConstant::membershipStatus(null, 'is_current_member = \'1\'', 'label')), ','),
          ),
        ),
        'grouping' => 'member-fields',
      ),
      ...
      'civicrm_email' => array(
        ...
        'fields' => array(
			'email' => array('required' => TRUE, 'default' => TRUE)
		),
        'grouping' => 'contact-fields',
      ),
	        'civicrm_participant' =>
	...
	//var_dump(CRM_Utils_Array::implodePadded(array_keys(CRM_Member_PseudoConstant::membershipStatus(null, 'is_current_member = \'1\'', 'label')), ',')); exit();
  }
  ...
  function where() {
    ...
    foreach ($this->_columns as $tableName => $table) {
      if (array_key_exists('filters', $table)) {
        foreach ($table['filters'] as $fieldName => $field) {
		  ...
		  if (isset($field['clause_type']) && $field['clause_type'] = 'or' && !empty($clause)) {
			$clauses_or[] = $clause;
		  } elseif (!empty($clause)) {
            $clauses[] = $clause;
          }
    ...
    if (empty($clauses)) {
      ...
      $this->_where = "WHERE {$this->_aliases['civicrm_participant']}.contact_id IS NULL AND {$this->_aliases['civicrm_email']}.contact_id IS NOT NULL AND ((" . implode(' AND ', $clauses) . ') OR ('.implode(' AND ', $clauses_or).'))';
    }
